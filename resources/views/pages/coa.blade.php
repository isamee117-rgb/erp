@extends('layouts.app')
@section('page-title', 'Chart of Accounts - LeanERP')
@section('content')

{{-- Save / Update Confirm Overlay --}}
<div id="coaSaveConfirm" class="d-none ms-overlay">
  <div class="ms-box">
    <div class="ms-body">
      <div class="ms-icon ms-icon-save"><i class="ti ti-device-floppy"></i></div>
      <div class="ms-title" id="coaSaveConfirmTitle">Save Account?</div>
      <div class="ms-sub" id="coaSaveConfirmSub">Please confirm to save this account.</div>
    </div>
    <div class="ms-footer">
      <button class="ms-btn-cancel" onclick="cancelCoaSave()">Cancel</button>
      <button class="ms-btn-ok" onclick="doSaveAccount()">Yes, Save</button>
    </div>
  </div>
</div>

{{-- Success Overlay --}}
<div id="coaSuccessOverlay" class="d-none ms-overlay">
  <div class="ms-box">
    <div class="ms-body">
      <div class="ms-icon ms-icon-success"><i class="ti ti-circle-check"></i></div>
      <div class="ms-title">Done!</div>
      <div class="ms-sub" id="coaSuccessMsg">Account saved successfully.</div>
    </div>
    <div class="ms-footer">
      <button class="ms-btn-ok" onclick="document.getElementById('coaSuccessOverlay').classList.add('d-none')">OK</button>
    </div>
  </div>
</div>

{{-- Delete Confirm Overlay --}}
<div id="coaDeleteConfirm" class="d-none ms-overlay">
  <div class="ms-box">
    <div class="ms-body">
      <div class="ms-icon ms-icon-confirm"><i class="ti ti-trash"></i></div>
      <div class="ms-title">Delete Account?</div>
      <div class="ms-sub">This account will be permanently removed. This action cannot be undone.</div>
    </div>
    <div class="ms-footer">
      <button class="ms-btn-cancel" onclick="cancelCoaDelete()">Cancel</button>
      <button class="ms-btn-confirm" onclick="doCoaDelete()">Yes, Delete</button>
    </div>
  </div>
</div>

{{-- Error Overlay --}}
<div id="coaErrorOverlay" class="d-none ms-overlay">
  <div class="ms-box">
    <div class="ms-body">
      <div class="ms-icon ms-icon-error"><i class="ti ti-alert-triangle"></i></div>
      <div class="ms-title">Cannot Delete</div>
      <div class="ms-sub" id="coaErrorMsg"></div>
    </div>
    <div class="ms-footer">
      <button class="ms-btn-ok" onclick="document.getElementById('coaErrorOverlay').classList.add('d-none')">OK</button>
    </div>
  </div>
</div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content pm-modal-content">
      <div class="modal-header pm-modal-header">
        <h5 class="modal-title pm-modal-title" id="coaModalLabel"><i class="ti ti-list-numbers me-2"></i>Add Account</h5>
      ...
      <div class="modal-body pm-modal-body">
        ...
        <div class="pm-field-row">
          <label class="pm-label">Account Code</label>
          <input type="text" class="form-control pm-input" id="coaCode" placeholder="e.g. 1001">
        </div>
        <div class="pm-field-row">
          <label class="pm-label">Account Name</label>
          <input type="text" class="form-control pm-input" id="coaName" placeholder="e.g. Cash in Hand">
        </div>
        <div class="pm-field-row">
          <label class="pm-label">Type</label>
          <select class="form-select pm-input" id="coaType" onchange="updateSubTypes()">
        ...
        <div class="pm-field-row">
          <label class="pm-label">Sub-Type</label>
          <select class="form-select pm-input" id="coaSubType"></select>
        </div>
      </div>
      <div class="modal-footer pm-modal-footer">
        <button type="button" class="pm-btn-cancel" data-bs-dismiss="modal">Cancel</button>
        <button type="button" class="pm-btn-save" onclick="confirmSave()"><i class="ti ti-device-floppy me-1"></i>Save Account</button>
      </div>
    </div>
  </div>
</div>
...
<div class="card inv-header-card">
  <div class="card-body inv-header-body">
    <div class="row align-items-center">
      <div class="col">
        <h2 class="mb-1 inv-title"><i class="ti ti-list-numbers me-2"></i>Chart of Accounts</h2>
...
<div class="card inv-section-card inv-table-card">
  <div class="table-responsive">
    <table class="table table-hover table-vcenter inv-table mb-0">
      <thead>
        <tr>
          <th class="inv-th coa-sort-th" onclick="toggleCodeSort()" title="Sort by code" style="cursor:pointer;user-select:none;">Code <span id="coaSortIcon">↑</span></th>
...
.pm-field-row{margin-bottom:16px;}.pm-field-row:last-child{margin-bottom:0;}
...
@endpush
@push('scripts')
<script src="{{ asset('js/pages/coa.js') }}?v={{ filemtime(public_path('js/pages/coa.js')) }}"></script>
@endpush